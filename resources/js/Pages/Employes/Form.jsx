...
export default function Form({ employe, clients }) {
  ...
  const { data, setData, post, processing, errors } = useForm({
    ...
    file_bpjs_kesehatan: null,
    no_bpjs_ketenaga: employe?.no_bpjs_ketenaga || "",
    file_bpjs_ketenaga: null,
  ...
  useEffect(() => {
    ...
    fetch(`/admin/employes/client-meta?client_id=${data.client_id}`)
      .then((r) => r.json())
      .then((meta) => {
        setData((values) => ({
          ...values,
  ...
  function submit(e) {
    e.preventDefault();
    post(editing ? `/admin/employes/${employe.id}` : "/admin/employes", {
      forceFormData: true,
    });
  }
        ...
        <Card>
          <CardHeader>
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <CardDescription>
                  Lengkapi identitas utama dan relasi mitra.
                </CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent className="grid gap-5 md:grid-cols-3">
            ...
            <TextField
              ...
              description={!editing ? "Terisi otomatis dari mitra." : undefined}
            />
            <TextField
              label="Nomor Urut"
              required
            ...
            <TextField
              ...
              type="date"
              error={errors.date_real}
              value={data.date_real}
              onChange={(e) => setData("date_real", e.target.value)}
            />
            <TextareaField
              label="Alamat"
          ...
          <CardHeader>
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <CardDescription>
                  Upload dokumen pendukung dan pilih jenis BPJS.
                </CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent className="grid gap-5 md:grid-cols-2">
            <FormField label="Foto KTP (Depan)" error={errors.img_ktp_dpn}>
              <Input
                type="file"
                onChange={(e) => setData("img_ktp_dpn", e.target.files[0])}
              />
            </FormField>
            <FormField label="Foto Profile" error={errors.img}>
              <Input
                type="file"
            ...
            <FormField
              ...
              error={errors.file_bpjs_kesehatan}
            >
              <Input
                type="file"
            ...
            <div className="space-y-2 md:col-span-2">
              ...
              <div className="flex flex-wrap gap-2">
                {bpjs.map((item) => (
                  ...
                  >
                    <Checkbox
                      checked={data.jenis_bpjs.includes(item)}
                      onCheckedChange={(checked) =>
                        setData(
                          "jenis_bpjs",
                          checked
                            ? [...data.jenis_bpjs, item]
                            : data.jenis_bpjs.filter((v) => v !== item),
                        )
                      }
                    />
                    {item.toUpperCase()}
